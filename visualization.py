...
def run_visualization(
    *, samplerate: int = 44100, blocksize: int = 2048, num_bars: int = 60, interp: float = 0.5
    ...
    def get_proj_view(angle, dist, elev):
        proj = np.array(
            Matrix44.perspective_projection(45, WIN_SIZE[0] / WIN_SIZE[1], 0.1, 100), dtype="f4"
        )
        eye = np.array(
            [
                np.sin(angle) * dist,
    ...
    while running:
        ...
        for i, (px, py) in enumerate(mesh_xy):
            ...
            min_dist = min(dists)
            scaled_dist = min_dist * 0.1
            rel = np.clip(1.0 - scaled_dist / 2.0, 0, 1)
            # Reverse the mapping: close = low bin, far = high bin
            bin_idx = int((1.0 - rel) * (len(fft_bars) - 1))
            raw = fft_bars[bin_idx]
            target = (raw ** 0.15) * 0.5  # further reduced scale for even lower mesh heights
